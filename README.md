...
- **Anti-bruteforce sur `/login`** (`Administration → Sécurité`) : après N échecs depuis une même
  adresse IP dans une fenêtre glissante, cette IP est bannie temporairement — bloquée avant même
  de vérifier les identifiants (HTTP 429), quel que soit l'utilisateur visé. Seuils (nombre
  d'échecs, fenêtre, durée du bannissement) et interrupteur marche/arrêt éditables depuis cette
  page, effectifs immédiatement sans redémarrage (désactiver débloque aussitôt une IP déjà
  bannie). À désactiver si l'instance tourne derrière un reverse proxy qui ne transmet pas
  `X-Forwarded-For` (sinon toutes les requêtes paraissent venir de l'IP du proxy — un
  bannissement bloquerait tout le monde) ou si l'accès est déjà maîtrisé autrement (réseau privé,
  VPN). Protection en mémoire, propre au process (cohérent avec le mono-worker gunicorn déjà
  requis par le stockage fichier) : équivalent applicatif à fail2ban, qui ne s'intègre pas au
  modèle de distribution en image Docker de ce projet.
...
| `LOGIN_MAX_ATTEMPTS` / `LOGIN_WINDOW_MINUTES` / `LOGIN_BAN_MINUTES` | non | Valeurs initiales de l'anti-bruteforce (`Administration → Sécurité`) au tout premier démarrage seulement — ensuite éditables depuis cette page, ces variables sont ignorées. Défauts : 5 / 5 / 15. |
